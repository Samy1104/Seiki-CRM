...
**Goal:** Integrate the sidebar design from `D:\Stage\SEIKI\Login Portal with Buttons` into `d:\Stage\SEIKI\Projet`, maintaining full design fidelity, animations, and linking all navigation items to application state.

**Architecture:** Create a unified `src/components/Sidebar.tsx` component in `d:\Stage\SEIKI\Projet` configured for both CRM and Contenu sections via props (`section="crm" | "contenu"`). Replace the old sidebar implementation in `src/App.tsx` and `src/views/Contenu.tsx`.

**Tech Stack:** React 19, Lucide React icons, Tailwind CSS / Vanilla CSS, React DOM (createPortal).

## Global Constraints
- Preserve exact sidebar design, colors (`#0d0d0d`, `#c8b89a`, `#5b5bd6`), typography, and transitions.
- Maintain responsive 220px expanded ↔ 56px collapsed layout.
- Use `createPortal` for floating avatar submenu when collapsed.

---

### Task 1: Create `src/components/Sidebar.tsx`

**Files:**
- Create: `d:\Stage\SEIKI\Projet\src\components\Sidebar.tsx`

**Interfaces:**
- Consumes: `useAuth` hook from `../context/AuthContext` for user profile and `logout()`.
- Produces: `Sidebar` React component accepting `SidebarProps`.

- [ ] **Step 1: Write `src/components/Sidebar.tsx` component**
...
git commit -m "feat: add new design Sidebar component"
```

---

### Task 2: Integrate `Sidebar` into `src/App.tsx` and `src/views/Contenu.tsx`

**Files:**
- Modify: `d:\Stage\SEIKI\Projet\src\App.tsx`
- Modify: `d:\Stage\SEIKI\Projet\src\views\Contenu.tsx`

- [ ] **Step 1: Update `src/App.tsx`**

Replace import `./components/SideBar` with `./components/Sidebar` and use:
`<Sidebar section="crm" currentView={currentView} setView={setView} setActiveApp={setActiveApp} />`

- [ ] **Step 2: Update `src/views/Contenu.tsx`**

Replace the legacy inline `<aside className="sidebar">` with `<Sidebar section="contenu" contenuView={contenuView} setContenuView={setContenuView} setActiveApp={setActiveApp} />`.
...
```bash
git add src/App.tsx src/views/Contenu.tsx
git commit -m "feat: integrate new Sidebar across CRM and Contenu views"
```
